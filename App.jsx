...
export default function EestiReisikaaslaneFull() {
  ...
  useEffect(() => {
    if (results) {
      // Kontrollime, kas Leaflet on juba laetud, kui ei, siis laeme CDN-i kaudu
      if (!window.L) {
  ...
  const initMap = () => {
    if (window.L && document.getElementById('map')) {
      // Eemaldame vana kaardi kui see eksisteerib
      const container = document.getElementById('map');
      if (container._leaflet_id) { container._leaflet_id = null; }
      
      const map = window.L.map('map').setView([59.4370, 24.7536], 13); // Tallinna keskpunkt
      window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '&copy; OpenStreetMap'
      }).addTo(map);

      // Lisame näidispunkti kaardile
